...
/*   Updated: 2021/04/15 18:23:45 by mvaldeta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pad_right_ints(int print, int to_pad, int min_c, int zero)
{
	...
	if (min_c == -1 && print == 0)
		return (padr_nomin_print0(print, to_pad, min_c, zero));
	if (min_c < -1)
		return (padr_nomin_generic(print, to_pad, min_c));
	if (zero != -1 && print < 0)
		return (padr_nozero_lessprint(print, to_pad, min_c));
	if (min_c == 0 && print == 0)
	...
	if (print == CHAR_MIN || print == CHAR_MAX)
		return (padr_chrminmax(print, to_pad, min_c));
	if (print == INT_MIN || print == INT_MAX)
		return (print_x_times((to_pad * -1) - 10, ' '));
	if (print < 0)
...
int	conv_itoa(char *input, int index, int has_format, va_list args2)
{
	...
	else
	{
		...
		if (input[index + 1] == ' ')
			ft_putc(' ');
		x = va_arg(args2, int);
		format_ints(x, var.to_pad, var.m, var.has_zero);
		return (ft_intstrchr_flag(input, 'i', index));
	}
	return (FAIL);
}
